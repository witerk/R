...
subset(iris, (Sepal.Length>4.8) & (Sepal.Length<5.1))[c('Sepal.Length','Sepal.Width')]

x = 1:100
sample(x, size=10, replace=F)  #비복원(replace=F 기본값) 추출
...
aggregate(mtcars[ , -c(2,8)],   #cyl,vs 없앤거(겹치니까)
          by = list(cyl=mtcars$cyl, vs=mtcars$vs),  #기준 칼럼 2개(이름 안 정하면 GROUP.1, 2로 뜸)
          max)   #FUN 없어도 됨
